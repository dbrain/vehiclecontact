...
function lookupRefreshToken(parts, refreshToken, cb) {
  ...
  parallel(calls, function checkRefreshToken(err, results) {
    if (err) {
      ...
      cb({ type: 'invalid_grant', message: 'Invalid refresh token' });
    } else {
...
function storeAccessToken(accessToken, request, result, cb) {
  ...
  doc.save(cleanBeforeCallback('apiKeys', cb));
}
...
function storeRefreshToken(refreshToken, request, result, cb) {
  ...
  doc.save(cleanBeforeCallback('refreshTokens', cb));
}
...
function replaceRefreshToken(refreshToken, newToken, doc) {
  ...
  if (updateMe) {
    ...
    doc.refreshTokens.push(newToken);
  }
  return doc;
}

function cleanBeforeCallback(tokenType, realCb) {
  return function handleSaveResult(err, doc) {
    if (err) {
      realCb(err, doc);
    } else {
      doc = cleanExpiredTokens(tokenType, doc);
      doc.save(function result(err) {
        if (err) {
...
function cleanExpiredTokens(tokenType, doc) {
  ...
  var removeMe = _.filter(tokens, function checkToken(token) {
    return currentDate >= token.expires;
  });
  _.each(removeMe, function remove(element) {
    tokens[tokens.indexOf(element)].remove();
  });
  return doc;
}
