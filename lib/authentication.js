...
function lookupRefreshToken(request, cb) {
  cb('lookupRefreshToken not_implemented');
}
...
function lookupAccessToken(parts, accessToken, cb) {
  lookupToken('apiKeys', parts, accessToken, cb);
}

function lookupToken(typeKey, parts, accessToken, cb) {
  ...
  _.each(lookups, function findAccessToken(id, type) {
    calls[type] = function lookupObject(lookupCb) {
      var query = User.findOne({});
      query.where('_id', id);
      query.where(typeKey + '.token', accessToken);
      query.run(lookupCb);
    };
  });
