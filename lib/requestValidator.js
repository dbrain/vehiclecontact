...
var validators = {
  'GET /': {
    ...
    queryParameters: [
      ...
      },
      {
        ...
        type: 'boolean',
        default: false,
        description: 'Should only conversations be returned'
      }
    ]
  },
  'GET /user/watching': {
    description: 'Get the regos this user is watching',
    returns: 'An array of watching items',
    queryParameters: [
      {
        name: 'count',
        type: 'integer',
        ...
        max: 100,
        default: 10,
        description: 'The number of regos to return'
      },
      {
        name: 'page',
        type: 'integer',
  ...
  'GET /regos': {
    description: 'Search regos',
    returns: 'An array of regos, flagged as watching: true if a user is logged in and watching',
    queryParameters: [
      {
        name: 'rego',
        type: 'string',
  ...
  'GET /rego/:country/:state/:rego': {
    description: 'Get a rego page',
    returns: 'A rego page including initial comments',
    errors: [
      {
        description: 'Rego not found',
        code: 404
      },
      {
        description: 'Request is in an invalid format.',
  ...
  return function parseRequest(req, res, next) {
    ...
    if (validator) {
      ...
      if (result.validation.count > 0) {
        res.send(result.validation, 400);
      } else {
...
function processValue(def, value) {
  if (value && def.processing) {
    return processingFns[def.processing](value);
  } else {
    return value;
  }
}
...
function validateString(result, def, value, resultObject) {
  ...
  if (!def.required && !value) {
    ...
    result[resultObject][def.name] = processValue(def, value);
  }
}
...
function validateBoolean(result, def, value, resultObject) {
  ...
  if (!def.required && !isBoolean) {
    ...
    result[resultObject][def.name] = processValue(def, actualValue);
  }
}

function validateInteger(result, def, value, resultObject) {
  var actualValue = parseInt(value, 10);
  var isNumber = _.isNumber(actualValue) && !_.isNaN(actualValue);
