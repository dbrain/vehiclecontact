      ...
      function makeTheMagicHappen(err, results) {
        if (err) {
          ...
          if (parentConversation) {
            ...
            rego.conversationCount.increment();
...
function queryNewsStream(queryParams, regos, cb) {
  ...
  if (queryParams.hideWatches) {
    query.where('type').ne('watch');
  }
  query.sort('_id', -1);
...
function findRegos(queryParams, cb) {
  var query = Rego.find({});
  if (queryParams.rego) {
    query.where('rego', queryParams.rego);
  }
...
function getRego(queryParams, regoNo, state, country, cb) {
  ...
  query.run(function regoResponse(err, rego) {
    var result = { result: null };
    if (rego) {
      result = {
        pagination: paginator.calculatePagination(rego.conversationCount, queryParams.page, queryParams.count, rego.conversations),
        result: rego
      };
    }
    cb(err, result);
